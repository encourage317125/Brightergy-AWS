  ...
  window.nv.models.tooltip = function() {
        ...
        */
        ...
        var contentGenerator = function(d) {
            ...
            var primaryDate = '', compareDate = '';
            if (d.value != '') {
                primaryDate = d.value;
                if (typeof d.compareValue !== 'undefined' && d.compareValue != '') {
                    compareDate = d.compareValue;
                }
            } else if (typeof d.compareValue !== 'undefined' && d.compareValue != '') {
                primaryDate = d.compareValue;
            }

            if(primaryDate != '') {
                var firstRow= tbodyEnter.select("tr");
                 firstRow.insert("tr", ":first-child")
                     .append("td")
                     .attr("colspan",3)
                     .append("strong")
                     .classed("x-value",true)
                     .html(headerFormatter(primaryDate));
            }
            if (compareDate != '') {
                var middleIdx = parseInt(d.series.length / 2);
                var middleRow= trowEnter.filter(":nth-child(" + middleIdx+ ")");
                middleRow.append("tr")
                    ...
                    .append("strong")
                    .classed("x-value",true)
                    .html(headerFormatter(compareDate));
            }
        ...
        var dataSeriesExists = function(d) {
            if (d && ((d.series && d.series.length > 0) || (
                typeof d.compareSeries !== 'undefined' && d.compareSeries && d.compareSeries.length > 0))) return true;
...
nv.utils.optionsFunc = function(args) {
  ...
  // Modification by Rumen, Nov 2014
  //------------------------------------------------------------
  ...
  axis
    ...
    .tickPadding(5) // Space between tick values and axis - 3 by default
    ;
    ...
    selection.each(function(data) {
      ...
      switch (axis.orient()) {
        ...
        case 'bottom':
          ...
          axisLabel.enter().append('text').attr('class', 'nv-axislabel');
          var w = (scale.range().length==2) ? scale.range()[1] : (scale.range()[scale.range().length-1]+(scale.range()[1]-scale.range()[0]));
          w = (scale.range().length==1) ? scale.range()[0] : w;
          axisLabel
              .attr('text-anchor', 'middle')
              .attr('y', xLabelMargin)
              .attr('x', w/2);
        ...
        case 'left':
          ...
          var yTicks = g.selectAll('g').select("text");
          yTicks.each(function(d,i){
            var labelPadding = this.getBBox().width + axis.tickPadding() + 16; //16: height of axis label
            if(labelPadding > width) width = labelPadding;
          });
      ...
      if (showMaxMin && (axis.orient() === 'left' || axis.orient() === 'right')) {
        ...
        g.selectAll('g') // the g's wrapping each tick
            .each(function(d,i) {
              ...
              if (scale(d) < scale.range()[1] + 15 || scale(d) > scale.range()[0] - 15) { // 15 is assuming text height is 11... if d is 0, leave it!
  ...
  function chart(selection) {
    selection.each(function(data) {
      ...
      var scatterWrap = wrap.select('.nv-scatterWrap');
          //.datum(data.filter(function(d) { return !d.isAggregation;})); // Data automatically trickles down from the wrap
