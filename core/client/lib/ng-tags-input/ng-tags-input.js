 ...
 */
tagsInput.directive('tagsInput', ["$timeout","$document","tagsInputConfig","$rootScope", function($timeout, $document, tagsInputConfig, $rootScope) {
    function TagList(options, events) {
        ...
        self.remove = function(index, sourceId, sourceType) {
            var tag = self.items.splice(index, 1)[0];
            events.trigger('tag-removed', { $tag: tag });
            $rootScope.removeSourceId = sourceId;
            $rootScope.removeSourceType = sourceType;
            $rootScope.$emit('CompanyGroupRemoved');
            return tag;
        };
        ...
        controller: ["$scope","$attrs","$element", function($scope, $attrs, $element) {
            ...
            this.registerAutocomplete = function() {
                ...
                return {
                    ...
                    sortTag: function() {
                        return $scope.tagList.items.sort(function(a,b){
 ...
 */
tagsInput.directive('autoComplete', ["$document","$timeout","$sce","tagsInputConfig","$rootScope", function($document, $timeout, $sce, tagsInputConfig, $rootScope) {
        ...
        link: function(scope, element, attrs, tagsInputCtrl) {
            ...
            scope.addSuggestion = function() {
                ...
                if (suggestionList.selected) {
                    $rootScope.newSourceId = suggestionList.selected.id;
                    $rootScope.newSourceType = suggestionList.selected.type;
                    $rootScope.$emit('CompanyGroupAdded');
...
tagsInput.run(["$templateCache", function($templateCache) {
    ...
    "<div class=\"host\" tabindex=\"-1\" ti-transclude-append=\"\"><div class=\"tags\" ng-class=\"{focused: hasFocus}\"><ul class=\"tag-list\"><li class=\"tag-item {{tag.type}}\" ng-repeat=\"tag in tagList.items track by $index\" id=\"{{tag.type}}_{{tag.id}}\" ng-class=\"{ selected: tag == tagList.selected }\"><span ng-bind=\"getDisplayText(tag)\"></span> <a class=\"remove-button\" ng-click=\"tagList.remove($index, tag.id, tag.type)\" ng-bind=\"options.removeTagSymbol\"></a></li></ul><input class=\"input\" ng-model=\"newTag.text\" ng-change=\"newTagChange()\" ng-trim=\"false\" ng-class=\"{'invalid-tag': newTag.invalid}\" ti-bind-attrs=\"{type: options.type, placeholder: options.placeholder, tabindex: options.tabindex}\" ti-autosize=\"\"></div></div>"
  );

  $templateCache.put('ngTagsInput/auto-complete.html',
    "<div class=\"autocomplete\" ng-show=\"suggestionList.visible\"><ul class=\"suggestion-list\"><li class=\"suggestion-item\" ng-repeat=\"item in suggestionList.items track by $index\" ng-class=\"{selected: item == suggestionList.selected}\" ng-click=\"addSuggestionByIndex($index)\" id=\"{{item.id}}_{{item.type}}\" ng-mouseenter=\"suggestionList.select($index)\" ng-bind-html=\"highlight(item)\"></li></ul></div>"
  );
}]);
